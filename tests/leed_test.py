...
def test_results_leed_option_one_with_shade_transmittance():
    folder = './tests/assets/leed/with_shade_transmittance/results'
...
def test_results_leed_option_one_with_shade_transmittance_file():
    folder = './tests/assets/leed/with_shade_transmittance/results'
    results = AnnualDaylight(folder)
    shd_file = './tests/assets/leed/with_shade_transmittance/shd.json'
...
def test_results_leed_option_one_with_states():
    folder = './tests/assets/leed/with_states/results'
    results = AnnualDaylight(folder)
    target_folder = Path('./tests/assets/temp/leed_summary')
    leed_option_one(results, use_states=True, sub_folder=target_folder)
    assert target_folder.is_dir()
    nukedir(target_folder, True)
